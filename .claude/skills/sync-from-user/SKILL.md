...
3. **Ask which to sync.** If multiple skills have changes, ask the user which to pull. Default to all, but let them opt out. Never sync silently without confirmation when there are real changes.

4. **Sync with rsync.** For each confirmed skill:
...
5. **Report what changed.** After syncing, run `git status skills/` (if this is a git repo) or `diff -rq` again against a fresh temp copy, and show the user a short summary of modified/added/removed files per skill. Do not commit — leave that to the user.

## Edge cases

- **Skill exists in user folder but not in repo.** That's a new skill the user authored live. Ask whether to import it as `skills/<name>/`. If yes, `cp -R ~/.claude/skills/<name> skills/<name>` and remind them to add it to [README.md](../../README.md)'s skill table.
...
- **Dirty working tree.** If `skills/` already has uncommitted changes in this repo, warn the user before overwriting — they may lose work. Ask before proceeding.

## Do not

- Do not run `rsync` without showing the diff first.
